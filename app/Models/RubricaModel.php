...
class RubricaModel extends Model
{
    ...
    // Timestamps
    protected $useTimestamps   = false; // Evita preenchimento automático de timestamps, manual via callbacks
    ...
    protected function setAuditoriaExclusao(array $data)
    {
        ...
        // Não use $this->update() aqui, senão você criará um loop infinito chamando beforeUpdate.
        // O campo _atualizado_por também é passado porque o trigger BEFORE DELETE
        // usa OLD._atualizado_por para registrar o deletado_por na tabela de histórico.
        if (!empty($data['id'])) {
            $this->builder()
