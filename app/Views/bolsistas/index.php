...
<?= $this->section('conteudo') ?>
                ...
                <?php foreach ($bolsistas as $b): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <button type="button" class="btn btn-sm btn-info shadow-sm" data-toggle="modal" data-target="#modalHistorico<?= $b['id_bolsista'] ?>">
    ...
    <!-- MODAIS DE HISTÓRICO (Renderizados a partir de bolsistas_historico) -->
    <?php if (!empty($bolsistas)): ?>
        <?php foreach ($bolsistas as $b): ?>
            <?php 
                $historicoBolsista = $historicosPorBolsista[$b['id_bolsista']] ?? [];
            ?>
            <div class="modal fade" id="modalHistorico<?= $b['id_bolsista'] ?>" tabindex="-1" role="dialog" aria-labelledby="modalHistoricoLabel<?= $b['id_bolsista'] ?>" aria-hidden="true">
                <div class="modal-dialog modal-xl" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-info text-white">
                            <h5 class="modal-title font-weight-bold" id="modalHistoricoLabel<?= $b['id_bolsista'] ?>">
                                <i class="fas fa-history mr-2"></i> Histórico de Alterações: <span class="text-warning"><?= esc($b['nome']) ?></span>
                            </h5>
                        ...
                        <div class="modal-body p-4">
                            ...
                            <div class="card border-left-success shadow-sm mb-4">
                                <div class="card-header bg-light py-2">
                                    <h6 class="m-0 font-weight-bold text-success">
                                        <i class="fas fa-check-circle mr-1"></i> Estado Atual do Bolsista (Tabela Principal)
                                    </h6>
                                </div>
                                <div class="card-body py-3">
                                    <div class="row">
                                        <div class="col-md-4 mb-2">
                                            ...
                                            <span class="font-weight-bold text-dark"><?= esc($b['nome']) ?></span>
                                        </div>
                                        <div class="col-md-4 mb-2">
                                            <small class="text-muted font-weight-bold d-block">CPF</small>
                                            <span class="text-dark"><?= esc($b['cpf']) ?></span>
                                        </div>
                                        <div class="col-md-4 mb-2">
                                            <small class="text-muted font-weight-bold d-block">E-MAIL</small>
                                            <span class="text-dark"><?= esc($b['email']) ?></span>
                                        </div>
                                        <div class="col-md-4 mb-2">
                                            <small class="text-muted font-weight-bold d-block">TELEFONE</small>
                                            <span class="text-dark"><?= esc($b['telefone'] ?? '-') ?></span>
                                        </div>
                                        <div class="col-md-4 mb-2">
                                            <small class="text-muted font-weight-bold d-block">DADOS BANCÁRIOS</small>
                                            <span class="text-dark"><?= esc(($b['banco'] ?? '-') . ' / Ag: ' . ($b['agencia'] ?? '-') . ' / CC: ' . ($b['conta_corrente'] ?? '-')) ?></span>
                                        </div>
                                        <div class="col-md-4 mb-2">
                            ...
                            <!-- SEÇÃO: Versões Anteriores (Shadow Table bolsistas_historico ordenada por _atualizado_em DESC) -->
                            ...
                            <?php if (empty($historicoBolsista)): ?>
                            ...
                            <?php else: ?>
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover shadow-sm">
                                        ...
                                        <tbody>
                                            <?php foreach ($historicoBolsista as $h): ?>
                                                <?php
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="small">
                                                        ...
                                                        <strong>Banco:</strong> <?= esc($h['banco'] ?? '-') ?> | <strong>Ag:</strong> <?= esc($h['agencia'] ?? '-') ?> | <strong>CC:</strong> <?= esc($h['conta_corrente'] ?? '-') ?>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
